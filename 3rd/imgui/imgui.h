...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoBordersInBodyUntilResize = 1 << 12,  // [ALPHA] Disable vertical borders in columns Body until hovered for onResized (borders will always appear in Headers). -> May move to style
...
// - Important: clear() frees memory, onResized(0) keep the allocated buffer. We use onResized(0) a lot to intentionally recycle allocated buffers across frames and amortize our costs.
...
struct ImGuiInputTextCallbackData
{
    ...
    // - To modify the text buffer in a callback, prefer using the InsertChars() / DeleteChars() function. InsertChars() will take care of calling the onResized callback if necessary.
    // - If you know your edits are not going to onResized the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal state.
