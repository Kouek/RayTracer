...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // When we are too small, start hiding and disabling the grab (this reduce visual noise on very small window and facilitate using the window onResized grab)
    float alpha = 1.0f;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    callback_buf = is_readonly ? buf : state->TextA.Data; // Pointer may have been invalidated by a onResized callback
                    ...
                    if (buf_dirty)
                    {
                        ...
                        if (callback_data.BufTextLen > backup_current_text_length && is_resizable)
                            state->TextW.resize(state->TextW.Size + (callback_data.BufTextLen - backup_current_text_length)); // Worse case scenario onResized
    ...
    if (apply_new_text != NULL)
    {
        ...
        // If the underlying buffer onResized was denied or not carried to the next frame, apply_new_text_length+1 may be >= buf_size.
