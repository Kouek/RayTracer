...
//    | - TableSetColumnWidth()                 - apply resizing width (for mouse onResized, often requested by previous frame)
...
//    | TableUpdateBorders()                    - detect hovering columns for onResized, ahead of contents submission
...
//   By default a table with _ScrollX but without _Resizable will have column auto-onResized.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    float sum_width_requests = 0.0f;        // Sum of all width for fixed and auto-onResized columns, excluding width contributed by Stretch columns but including spacing/padding.
...
void    ImGui::EndTable()
{
    ...
    // Update ColumnsAutoFitWidth to get us ahead for host using our size to auto-onResized without waiting for next BeginTable()
    ...
    // Override declared contents width/height to enable auto-onResized while not needlessly adding a scrollbar
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // Qt when manual onResized is enabled only support a single _trailing_ stretch column.

    // When forwarding onResized from Wn| to Fn+1| we need to be considerate of the _NoResize flag on Fn+1.
    // FIXME-TABLE: Find a way to rewrite all of this so interactions feel more consistent for the user.
    // Scenarios:
    // - F1 F2 F3  onResized from F1| or F2|   --> ok: alter ->WidthRequested of Fixed column. Subsequent columns will be offset.
    // - F1 F2 F3  onResized from F3|          --> ok: alter ->WidthRequested of Fixed column. If active, ScrollX extent can be altered.
    // - F1 F2 W3  onResized from F1| or F2|   --> ok: alter ->WidthRequested of Fixed column. If active, ScrollX extent can be altered, but it doesn't make much sense as the Stretch column will always be minimal size.
    // - F1 F2 W3  onResized from W3|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 W2 W3  onResized from W1| or W2|   --> ok
    // - W1 W2 W3  onResized from W3|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 F2 F3  onResized from F3|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 F2     onResized from F2|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 W2 F3  onResized from W1| or W2|   --> ok
    // - W1 F2 W3  onResized from W1| or F2|   --> ok
    // - F1 W2 F3  onResized from W2|          --> ok
    // - F1 W3 F2  onResized from W3|          --> ok
    // - W1 F2 F3  onResized from W1|          --> ok: equivalent to resizing |F2. F3 will not move.
    // - W1 F2 F3  onResized from F2|          --> ok
    // All resizes from a Wx columns are locking other columns.

    // Possible improvements:
    // - W1 W2 W3  onResized W1|               --> to not be stuck, both W2 and W3 would stretch down. Seems possible to fix. Would be most beneficial to simplify onResized of all-weighted columns.
    // - W3 F1 F2  onResized W3|               --> to not be stuck past F1|, both F1 and F2 would need to stretch down, which would be lossy or ambiguous. Seems hard to fix.

    // [Resize Rule 1] Can't onResized from right of right-most visible column if there is any Stretch column. Implemented in TableUpdateLayout().

    // If we have all Fixed columns OR resizing a Fixed column that doesn't come after a Stretch one, we can do an offsetting onResized.
    // This is the preferred onResized path
...
void ImGui::EndColumns()
{
    ...
    // Draw columns borders and handle onResized
    // The IsBeingResized flag ensure we preserve pre-onResized columns width so back-and-forth are not lossy
