...
template <typename T, int numericalBase = 0> class NumericalBase {
  public:
    /// This constructor required for correct AgrumentCountChecker
    /// initialization
...
class Parser {
  private:
    ...
    static bool parse(const std::vector<std::string> &elements,
                      const bool &defval) {
        if (elements.size() != 0)
            throw std::runtime_error("A boolean command line parameter "
    ...
    static float parse(const std::vector<std::string> &elements,
                       const float &) {
        if (elements.size() != 1)
            throw std::bad_cast();
    ...
    static unsigned int parse(const std::vector<std::string> &elements,
        ...
        return static_cast<unsigned int>(
            std::stoul(elements[0], 0, numberBase));
    }
...
#pragma warning(pop)
    ...
    void set_default(bool is_required, const std::string &description = "") {
        auto command =
            new CmdArgument<T>{"", "", description, is_required, false};
    ...
    template <typename T>
    void set_optional(const std::string &name, const std::string &alternative,
    ...
    bool run(std::ostream &output, std::ostream &error) {
        if (_arguments.size() > 0) {
            ...
            for (size_t i = 0, n = _arguments.size(); i < n; ++i) {
                auto isarg =
                    _arguments[i].size() > 0 && _arguments[i][0] == '-';
                ...
                if (associated != nullptr) {
                    ...
                    if (!current->variadic) {
                        // If the current command is not variadic, then no
                        // more arguments should be added to it. In this
                        // case, switch back to the default command.
        ...
        // First, parse dominant arguments since they succeed even if
    ...
    template <typename T> T get(const std::string &name) const {
        for (const auto &command : _commands) {
            if (command->name == name) {
                ...
                if (cmd == nullptr) {
                    throw std::runtime_error("Invalid usage of the parameter " +
                                             name + " detected.");
                }
        ...
        throw std::runtime_error("The parameter " + name +
                                 " could not be found.");
    }
